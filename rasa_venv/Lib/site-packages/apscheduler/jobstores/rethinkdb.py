...
class RethinkDBJobStore(BaseJobStore):
    ...
    def start(self, scheduler, alias):
        ...
        if self.database not in self.r.run(self.conn):
            self.r.run(self.conn)

        if self.table_name not in self.r.run(self.conn):
            self.r.run(self.conn)

        if 'next_run_time' not in self.r.table(self.table_name).run(self.conn):
            self.r.table(self.table_name).run(self.conn)

        self.table = self.r.db(self.database).table(self.table_name)

    def lookup_job(self, job_id):
        results = list(self.table.get_all(job_id).run(self.conn))
    ...
    def add_job(self, job):
        ...
        results = self.table.run(self.conn)
        if results['errors'] > 0:
    ...
    def _get_jobs(self, predicate=None):
        ...
        query = query.order_by('next_run_time', 'id')
        ...
        if failed_job_ids:
            self.r.expr(failed_job_ids).run(self.conn)
